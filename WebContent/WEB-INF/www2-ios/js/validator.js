/**
 * Form Validator Author : ok8008@yeah.net Link :
 * https://github.com/liutian1937/Validator Version : v4.1
 */
(function() {
	var defaults = {
		together : false, // 默认遇错误打断，显示单条错误信息
		errShow : 'alert', // 错误提示，默认为alert，支持字符串(alert,single,multiple),自定义function(string
		// || array())
		errBox : 'error_strings', // 错误消息class，默认为form表单中的 .error_strings
		errPar : 'li', // 单个表单元素的父级元素，用于定位错误的位置 li > (span > input ) ~
		// span.error_strings
	...
	var Common = {
		...
		getPosTop : function(obj) {
			return obj.offsetParent ? (obj.offsetTop + arguments
					.callee(obj.offsetParent)) : obj.offsetTop;
		},
		getParent : function(obj, tagname) {
			return obj.parentNode.tagName == tagname ? obj.parentNode
					: arguments.callee(obj.parentNode, tagname);
		},
			...
			if (window.addEventListener) {
				return function(el, identifier) {
					if (identifier) {
						...
						for (key in el.bindFn) {
							el.removeEventListener(el.bindFn[key]['eventType'],
									el.bindFn[key]['eventFn']);
						}
					}
					;
				};
				...
				return function(el, identifier) {
					if (identifier) {
						...
						var key;
						for (key in el.bindFn) {
							el.detachEvent("on" + el.bindFn[key]['eventType'],
									el.bindFn[key]['eventFn']);
						}
					}
				};
	...
	Validator.fn.destory = function() {
		var self = this;
		var inputList = self.formobj.getElementsByTagName('input'), selectList = self.formobj
				.getElementsByTagName('select'), textareaList = self.formobj
	...
	Validator.fn.showErrorSingle = function(data) {
		...
		if (!self.errObjList[name]) {
			...
			self.errObjList[name] = Common.getErrBox(obj, 'span',
					self.options.errBox);
		}
	...
	var Commands = function(data) {
		...
		case 'required':
			...
			break;
		case 'maxlength':
			return Common.isEmpty(data.obj.value) ? true
					: data.obj.value.length <= data.ruleExt;
			break;
		case 'minlength':
			return Common.isEmpty(data.obj.value) ? true
					: data.obj.value.length >= data.ruleExt;
			break;
		case 'number':
			return Common.isEmpty(data.obj.value) ? true : /^[\d]+$/
					.test(data.obj.value);
			break;
		case 'alpha':
			return Common.isEmpty(data.obj.value) ? true : /^[A-Za-z]+$/
					.test(data.obj.value);
			break;
		case 'string':
			return Common.isEmpty(data.obj.value) ? true : /^\w+$/
					.test(data.obj.value);
			break;
		case 'email':
			return Common.isEmpty(data.obj.value) ? true
		...
		case 'mobile':
			return Common.isEmpty(data.obj.value) ? true
					: /^0?1(3|4|5|8)\d{9}$/.test(data.obj.value);
			break;
		case 'url':
		...
		case 'lessthan':
		case 'greaterthan':
			var compare = typeof data.ruleExt == 'number' ? data.ruleExt
					: parseInt(this.formobj[data.ruleExt].value);
			return method == 'lessthan' ? parseInt(data.obj.value) < compare
					: parseInt(data.obj.value) > compare;
			break;
		case 'equal':
		case 'unequal':
			var val = data.obj.value, ext = data.ruleExt, self = this;
			return typeof ext == 'number' ? (function() {
				return method == 'equal' ? parseInt(val) == ext
						: parseInt(val) != ext;
			})() : (function() {
				if (ext.constructor === Array) {
					return method == 'equal' ? Common.inArray(val, ext)
							: !Common.inArray(val, ext);
				} else {
					return method == 'equal' ? val == self.formobj[ext].value
							: val != self.formobj[ext].value;
				}
			})();
			break;
		case 'notselect':
			...
			if (objArr.length) {
				if (data.ruleExt && data.ruleExt.constructor === Array) {
					...
					for (var i = 0; i < objArr.length; i += 1) {
						if (objArr[i].checked == true
								&& Common
										.inArray(objArr[i].value, data.ruleExt)) {
							retNum += 1;
						}
					}
					;
		...
		case 'maxselect':
			...
			return method == 'minselect' ? checklen >= data.ruleExt
					: checklen <= data.ruleExt;
			break;
		default:
			return false;
			break;
