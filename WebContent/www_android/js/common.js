...
function checkMobile(mobile) {
    if (mobile.length == 11 && /^(((13[0-9]{1})|(14[0-9]{1})|(15[0-9]{1})|(18[0-9]{1}))+\d{8})$/
        .test(mobile)) {
        return true;
    } else {
        return false;
 ...
 */
//注册
function registSms(event) {
        var mobile = $(event).prev().val();
...
$(function() {
    ...
    $("#news").on("panelload", function() {
        appAjax({
            type: "POST",
            url: "newsinfo/catelist/?pageNo=1",
            context: $("#news"),
            success: function(temp) {
                if (temp) {
                    $("#jtlg-pub-pic").empty();
    ...
    //注册
    $("#reg .reg-btn").click(function(event) {
        var phoneReg = /^1[3|4|5|8][0-9]\d{8}$/;
        if (!$("#reg").find("input[name='mobile']").val()) {
            $.afui.popup({
                message: "请输入手机号！",
                suppressTitle: true,
                cancelText: "知道了",
        ...
        } else if (!phoneReg.test($("#reg").find("input[name='mobile']").val())) {
            $.afui.popup({
                message: "请输入有效的手机号！",
                suppressTitle: true,
                cancelText: "知道了",
        ...
        } else if (!$("#reg").find("input[name='yzm']").val()) {
            $.afui.popup({
                message: "请输入验证码！",
                suppressTitle: true,
                cancelText: "知道了",
        ...
        } else if (!$("#reg").find("input[name='userName']").val()) {
            $.afui.popup({
                message: "请输入昵称！",
                suppressTitle: true,
                cancelText: "知道了",
        ...
        } else if (!$("#reg").find("input[name='password']").val()) {
            $.afui.popup({
                message: "请输入密码！",
                suppressTitle: true,
                cancelText: "知道了",
        ...
        } else if (!$("#reg").find("input.re-password").val()) {
            $.afui.popup({
                message: "请再次输入密码！",
                suppressTitle: true,
                cancelText: "知道了",
        ...
        } else if ($("#reg").find("input[name='password']").val() !== $("#reg").find("input.re-password").val()) {
            $.afui.popup({
                message: "两次输入密码不一致！",
                suppressTitle: true,
                cancelText: "知道了",
        ...
        } else if ($("#reg select.level1").children('option:selected').val() == 0) {
            $.afui.popup({
                message: "请选择一级分类！",
                suppressTitle: true,
                cancelText: "知道了",
        ...
        } else if ($("#reg select[name='cateIds']").children('option:selected').val() == 0) {
            $.afui.popup({
                message: "请选择二级分类！",
                suppressTitle: true,
                cancelText: "知道了",
            ...
            appAjax({
                ...
                data: {
                    cateIds: $("#reg select[name='cateIds']").children('option:selected').val()
                },
                success: function(temp) {
                    console.log(temp);
                    if (temp.isSuccess) {
