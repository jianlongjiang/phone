...
import com.phone.cn.conf.enums.BooleanEnum;
import com.phone.cn.entity.BaseEntity;
import com.phone.cn.entity.search.SearchOperator;
import com.phone.cn.entity.search.Searchable;
import com.phone.cn.plugin.entity.Treeable;
import com.phone.cn.plugin.service.BaseTreeableService;
import com.phone.cn.plugin.web.controller.entity.ZTree;
...
public abstract class BaseTreeableController<M extends BaseEntity<ID> & Treeable<ID>, ID extends Serializable>
    ...
    public String updateForm(@PathVariable("id") M m, Model model, RedirectAttributes redirectAttributes) {
        ...
        if (m == null) {
            redirectAttributes.addFlashAttribute(DataConfig.ERROR, "您修改的数据不存在！");
    ...
    public String update(
        ...
        redirectAttributes.addFlashAttribute(DataConfig.MESSAGE, "修改成功");
        return redirectToUrl(viewName("success"));
    }
    ...
    public String deleteInBatch(
            ...
            @RequestParam(value = DataConfig.BACK_URL, required = false) String backURL,
            RedirectAttributes redirectAttributes) {
        ...
        for (M m : mList) {
            if (m.isRoot()) {
                redirectAttributes.addFlashAttribute(DataConfig.ERROR, "您删除的数据中包含根节点，根节点不能删除");
    ...
    public String appendChildForm(@PathVariable("parent") M parent, Model model) {
        ...
        model.addAttribute(DataConfig.OP_NAME, "添加子节点");
    ...
    public String appendChild(
        ...
        redirectAttributes.addFlashAttribute(DataConfig.MESSAGE, "添加子节点成功");
        return redirectToUrl(viewName("success"));
    }
    ...
    public String showMoveForm(
        ...
        model.addAttribute(DataConfig.OP_NAME, "移动节点");
    ...
    public String move(
        ...
        if (target.isRoot() && !moveType.equals("inner")) {
            model.addAttribute(DataConfig.ERROR, "不能移动到根节点之前或之后");
            return showMoveForm(request, async, source, searchable, model);
        }

        baseService.move(source, target, moveType);

        redirectAttributes.addFlashAttribute(DataConfig.MESSAGE, "移动节点成功");
        return redirectToUrl(viewName("success"));
    }
    ...
    private ZTree<ID> convertToZtree(M m, boolean open, boolean onlyCheckLeaf) {
